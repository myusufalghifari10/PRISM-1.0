...
abx = pd.read_csv("../Data/processed_files/abx.csv", sep="|")
culture = pd.read_csv("../Data/processed_files/culture.csv", sep="|")
microbio = pd.read_csv("../Data/processed_files/microbio.csv", sep="|")
demog = pd.read_csv("../Data/processed_files/demog.csv", sep="|")
ce010 = pd.read_csv("../Data/processed_files/ce010000.csv", sep="|")
ce1020 = pd.read_csv("../Data/processed_files/ce1000020000.csv", sep="|")
ce2030 = pd.read_csv("../Data/processed_files/ce2000030000.csv", sep="|")
ce3040 = pd.read_csv("../Data/processed_files/ce3000040000.csv", sep="|")
ce4050 = pd.read_csv("../Data/processed_files/ce4000050000.csv", sep="|")
ce5060 = pd.read_csv("../Data/processed_files/ce5000060000.csv", sep="|")
ce6070 = pd.read_csv("../Data/processed_files/ce6000070000.csv", sep="|")
ce7080 = pd.read_csv("../Data/processed_files/ce7000080000.csv", sep="|")
ce8090 = pd.read_csv("../Data/processed_files/ce8000090000.csv", sep="|")
ce90100 = pd.read_csv("../Data/processed_files/ce90000100000.csv", sep="|")
MV = pd.read_csv("../Data/processed_files/mechvent.csv", sep="|")
inputpreadm = pd.read_csv("../Data/processed_files/preadm_fluid.csv", sep="|")
inputMV = pd.read_csv("../Data/processed_files/fluid_mv.csv", sep="|")
inputCV = pd.read_csv("../Data/processed_files/fluid_cv.csv", sep="|")
vasoMV = pd.read_csv("../Data/processed_files/vaso_mv.csv", sep="|")
vasoCV = pd.read_csv("../Data/processed_files/vaso_cv.csv", sep="|")
UOpreadm = pd.read_csv("../Data/processed_files/preadm_uo.csv", sep="|")
UO = pd.read_csv("../Data/processed_files/uo.csv", sep="|")
labU = [
    pd.read_csv("../Data/processed_files/labs_ce.csv", sep="|"),
    pd.read_csv("../Data/processed_files/labs_le.csv", sep="|"),
]
...
subj_dischtime_list = (
    demog.sort_values(by="admittime")
    .groupby("subject_id")
    .apply(lambda df: np.unique(df.dischtime.values))
)  # Create list of discharge times for each patient (output is a dict keyed by 'subject_id')
...
for i in bacterio.index.tolist():
    ...
    if np.isnan(bacterio.loc[i, "icustay_id"]):
        ...
        jj = demog.index[
            (demog["subject_id"] == subjectid) & (demog["hadm_id"] == hadmid)
        ].tolist()
        for j in range(len(ii)):
            if (o >= demog.loc[ii[j], "intime"] - 48 * 3600) and (
                o <= demog.loc[ii[j], "outtime"] + 48 * 3600
            ):
                bacterio.loc[i, "icustay_id"] = demog.loc[ii[j], "icustay_id"]
            elif (
                len(ii) == 1
            ):  # If we cant confirm from admission and discharge time but there is only 1 admission: it's the one!!
                bacterio.loc[i, "icustay_id"] = demog.loc[ii[j], "icustay_id"]
...
for i in abx.index.tolist():
    ...
    if np.isnan(abx.loc[i, "icustay_id"]):
        ...
        for j in range(len(ii)):
            if (
                o >= demog.loc[ii[j], "intime"] - 48 * 3600
                and o <= demog.loc[ii[j], "outtime"] + 48 * 3600
            ):
...
for icustayid in icustayidlist:
    ...
    ab = abx.loc[
        abx["icustay_id"] == icustayid, "startdate"
    ]  # Start time of abx for this icustayid
    bact = bacterio.loc[
        bacterio["icustay_id"] == icustayid, "charttime"
    ]  # Time of sample
    ...
    if len(ab) > 0 and len(bact) > 0:  # If we have data for both: proceed
        ...
        D = (
            cdist(
                ab.values.reshape(ab.values.shape[0], 1),
                bact.values.reshape(bact.values.shape[0], 1),
            )
            / 3600
        )
        for i in range(
            D.shape[0]
        ):  # looping through all rows of adminsitered antibiotics, from early to late
...
for r in Reflabs_values:
    try:
        Reflabs_id_dict[r] = (
            np.max(np.where(Reflabs.values == r)[0]) + 1
        )  # for row: +1 due to Index correction python
    except:
        print(r)
        break
...
for r in Refvitals_values:
    Refvitals_id_dict[r] = (
        np.max(np.where(Refvitals.values == r)[0]) + 1
    )  # +1 due to index correction for Python from MATLAB
ce_dfs = [
    ce010,
    ce1020,
    ce2030,
    ...
    if qst > 0:  # if we have a flag
        ...
        if d1[0] > 6574:  # If older than 18 years old
            ...
            ii = (temp["charttime"] >= qst - (winb4 + 4) * 3600) & (
                temp["charttime"] <= qst + (winaft + 4) * 3600
            )  # Time period of interest -4h and +4h
            ...
            ii = (temp2["charttime"] >= qst - (winb4 + 4) * 3600) & (
                temp2["charttime"] <= qst + (winaft + 4) * 3600
            )  # Time period of interest -4h and +4h
            ...
            ii = (temp3["charttime"] >= qst - (winb4 + 4) * 3600) & (
                temp3["charttime"] <= qst + (winaft + 4) * 3600
            )  # Time period of interest -4h and +4h
            temp3 = temp3.loc[ii]  # only time period of interest
            t = np.unique(
                pd.concat(
                    [temp["charttime"], temp2["charttime"], temp3["charttime"]],
                    ignore_index=True,
                ).values
            ...
            if len(t) > 0:
                for i in range(len(t)):
                    ...
                    reformat[irow, 2 + col.astype(int).values] = (
                        value.values
                    ...
                    reformat[irow, 30 + col.astype(int).values] = (
                        value.values
                    )  # Store available values
                ...
                qstime[icustayid][0] = (
                    qst  # Flag for presumed infection / this is time of sepsis if SOFA >=2 for this patient
                )
...
# FiO2
reformat[(~np.isnan(reformat[:, 22])) & (np.isnan(reformat[:, 23])), 23] = (
    reformat[(~np.isnan(reformat[:, 22])) & (np.isnan(reformat[:, 23])), 22] / 100
)
reformat[(~np.isnan(reformat[:, 23])) & (np.isnan(reformat[:, 22])), 22] = (
    reformat[(~np.isnan(reformat[:, 23])) & (np.isnan(reformat[:, 22])), 23] * 100
)
...
ii = np.where(
    (np.isnan(reformatsah[:, 22]))
    & (~np.isnan(reformatsah[:, 24]))
    & ((reformatsah[:, 21] == 0) | (reformatsah[:, 21] == 2))
)[0]  # As np.where returns a tuple
...
for i in range(len(icustayidlist)):
    ...
    rate = input[
        "norm_rate_of_infusion"
    ]  # rate of infusion (is NaN for boluses) || corrected for tonicity
    pread = inputpreadm[inputpreadm["icustay_id"] == icustayid][
        "inputpreadm"
    ]  # preadmission volume
    ...
    bolus = np.nansum(
        input[np.isnan(input[:, 5]) & (input[:, 1] >= t0) & (input[:, 1] <= t1), 6]
    ) + np.nansum(input2[(input2[:, 1] >= t0) & (input2[:, 1] <= t1), 4])
    ...
    dem = np.array(
        ...
        + list(
            abs(demog.dod[demogi].values - demog.outtime[demogi].values)
            < (24 * 3600 * 2)
        )
    ...
    pread = UOpreadm[UOpreadm["icustay_id"] == icustayid - 200000][
        "value"
    ].values  # preadmission UO ????????????????? Why no + 200000 for icustayid here?
    if len(pread) > 0:  # store the value, if available
    ...
    UOnow = np.nansum(
        output[(output["charttime"] >= t0) & (output["charttime"] <= t1)][
            "value"
        ].values
    )  # t0 and t1 defined above
    ...
    for j in range(0, 79, timestep):  # 0:timestep:79 #-28 until +52 = 80 hours in total
        ...
        ii = (temp[:, 2] >= t0) & (
            temp[:, 2] <= t1
        )  # index of items in this time period
        if sum(ii) > 0:
            ...
            if sum(ii) == 1:  # if only 1 row of values at this timestep
                ...
                reformat2[irow, 11:78] = np.nanmean(
                    value[:, 3:], axis=0
            ...
            # CareVue
            v2 = vaso2[(vaso2["charttime"] >= t0) & (vaso2["charttime"] <= t1)][
                "rate_std"
            ].values
            ...
            infu = np.nansum(
                ...
                + rate
                * (endt - t0)
                * ((startt <= t0) & (endt <= t1) & (endt >= t0))
                / 3600
                + rate
                * (t1 - startt)
                * ((startt >= t0) & (endt >= t1) & (startt <= t1))
                / 3600
                + rate * (t1 - t0) * ((endt >= t1) & (startt <= t0)) / 3600
            )
            # All boluses received during this timestep, from inputMV (need to check rate is NaN) and inputCV (simpler):
            bolus = np.nansum(
                input[
                    (np.isnan(input[:, 5])) & (input[:, 1] >= t0) & (input[:, 1] <= t1),
                    6,
                ]
            ) + np.nansum(input2[(input2[:, 1] >= t0) & (input2[:, 1] <= t1), 4])
...
reformat3t = pd.DataFrame(
    reformat3,
    columns=reformat2t.columns[
        [True] * 11 + (miss[11:74] < 0.7).tolist() + [True] * 11
    ],
)
...
for i in range(
    0, reformat3.shape[0], 9999
):  # dataset divided in 10K rows chunks (otherwise too large)
    bar_knn.update()
...
reformat4t["elixhauser"] = reformat4t["elixhauser"].fillna(
    np.nanmedian(reformat4t["elixhauser"])
)
...
)  # count of points for all 6 criteria of sofa
s2 = np.array(
    [
        s[:, 1] > 150,
        (s[:, 1] >= 100) & (s[:, 1] < 150),
...
for i in range(num_of_trials):
    if d["mortality_90d"].iloc[i] == 1:
        ii = (reformat4t["icustayid"] == d.index[i]) & (
            reformat4t["bloc"] == d_count.iloc[i]["mortality_90d"]
        )  # last row for this patient
        e[i] = (
            np.sum(
                (reformat4t["max_dose_vaso"][ii] == 0)
                & (d["vaso"].iloc[i] > 0.3)
                & (reformat4t["SOFA"][ii] >= d["sofa"].iloc[i] / 2)
            )
            > 0
        )
...
ii = (
    (reformat4t["bloc"] == 1)
    & (reformat4t["died_within_48h_of_out_time"] == 1)
...
sepsis = np.zeros(
    (30000, 5)
)  # NOTE: For other cohorts, this size may have to be changed
irow = 0
...
for icustayid in icustayidlist:
    ...
    if sum(ii) > 0:
        ...
        sepsis[irow, 4] = qstime[icustayid][
            0
        ]  # time of onset of sepsis #icustayid-1 not done to keep it consistent with earlier verified use of qstime and 0 added as onset of sepsis index.
        irow += 1
...
if pargs.save_intermediate:
    sepsis.to_csv("../Data/new_sepsis_mimiciii.csv", index=False)
...
print(
    "Sepsis Cohort -- Making an array with all unique charttime (1 per row) and all items in columns."
)
...
for icustayidrow in range(1, sepsis.shape[0] + 1):
    bar.update()
    qst = sepsis["sepsis_time"].iloc[
        icustayidrow - 1
    ]  # ;%,3); %flag for presumed infection
    ...
    if len(t) > 0:
        for i in range(len(t)):
            ...
            reformat[irow, 3 + col.astype(int).values] = (
                value.values
            )  # Store available values
            ...
            col = temp2.loc[ii, "itemid"]
            value = temp2.loc[ii, "valuenum"]
            reformat[irow, 31 + col.astype(int).values] = (
                value.values
            )  # store available values
        ...
        qstime[icustayid][1] = t[0]  # First timestamp
        qstime[icustayid][2] = t[-1]  # Last timestamp
        qstime[icustayid][3] = demog.loc[
            demog["icustay_id"] == icustayid, "dischtime"
        ].values[0]  # Discharge time
...
# Temperature
reformat[(reformat[:, 14] > 90) & (np.isnan(reformat[:, 15])), 15] = reformat[
    (reformat[:, 14] > 90) & (np.isnan(reformat[:, 15])), 14
]
...
# FiO2
reformat[(~np.isnan(reformat[:, 23])) & (np.isnan(reformat[:, 24])), 24] = (
    reformat[(~np.isnan(reformat[:, 23])) & (np.isnan(reformat[:, 24])), 23] / 100
)
reformat[(~np.isnan(reformat[:, 24])) & (np.isnan(reformat[:, 23])), 23] = (
    reformat[(~np.isnan(reformat[:, 24])) & (np.isnan(reformat[:, 23])), 24] * 100
)

# ESTIMATE FiO2 /// with use of interface / device (cannula, mask, ventilator....)
print("Sepsis Cohort -- Doing sample and hold")
reformatsah = SAH(
    reformat, sample_and_hold, adjust=1
)  # do SAH first to handle this task, setting `adjust` to 1 since we added another column

# NO FiO2, YES O2 flow, no interface OR cannula
ii = np.where(
    (np.isnan(reformatsah[:, 23]))
    & (~np.isnan(reformatsah[:, 25]))
    & ((reformatsah[:, 22] == 0) | (reformatsah[:, 22] == 2))
)[0]  # As np.where returns a tuple
...
)[0]  # No FiO2 given and O2 flow given, no interface OR cannula
...
reformat = SAH(
    reformat[:, 0:69], sample_and_hold, adjust=1
)  # Setting `adjust` to 1 to account for the added column for `presumed_onset`
#######################################################################
########################################################################
#                             DATA COMBINATION
########################################################################
...
for i in range(len(icustayidlist)):
    ...
    dem = np.array(
        ...
        + list(demog.elixhauser[demogi].values)
        + list(
            demog.re_admission[demogi].values
        )  # Using corrected readmission (within 30 days of previous discharge)
        ...
        + [
            (qstime[icustayid][3] - qstime[icustayid][2]) / 3600
        ]  # Length of time after last observation and discharge?
    )
    ...
    for j in range(
        0, 79, timestep
    ):  # 0:timestep:79 % -28 until +52 = 80 hours in total (4 hours buffer on either side of our desired window)
        t0 = 3600 * j + beg  # left limit of time window
        t1 = 3600 * (j + timestep) + beg  # right limit of time window
        ii = (temp[:, 2] >= t0) & (
            temp[:, 2] <= t1
        ...
        if sum(ii) > 0:
            ...
            if sum(ii) == 1:  # if only 1 row of values at this timestep
                ...
                reformat2[irow, 12:79] = np.nanmean(
                    value[:, 4:], axis=0
                )  # mean of all available values
...
# FIX Elixhauser missing values
reformat3t["elixhauser"] = reformat3t["elixhauser"].fillna(
    np.nanmedian(reformat3t["elixhauser"])
)
...
miss = pd.DataFrame(
    [np.sum(np.isnan(reformat3t.values), axis=0) / reformat3t.shape[0]],
    columns=reformat3t.columns,
)
...
MIMICtable.to_csv("../Data/MIMICtable.csv", index=False)
print('  → Saved checkpoint: ../Data/MIMICtable.csv')
...
vc = np.array(MIMICtable["max_dose_vaso"].values, copy=True)
...
ob_df = pd.DataFrame(
    MIMICzs[colbin + colnorm + collog].values, columns=colbin + colnorm + collog
)
...
for i in trajectories:
    ...
    data["traj"][i]["actions"] = ac_df[ac_df["traj"] == i]["action"].values.astype(
        np.int32
    )
