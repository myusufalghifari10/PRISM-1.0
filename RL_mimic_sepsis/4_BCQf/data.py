...
# ── Shifted-alignment loaders ──
# The original EpisodicBuffer / SASRBuffer loading pattern (with +1 offset on
# actions, rewards, notdone, next_state) ALREADY produces the correct causal
# transitions. The shifted episodic tensor simply stores pre-shifted actions.
# When loaded with the original +1 offset, the result is identical to the
# original BCQf training flow — no overshift, terminal reward preserved.
#
# Alias the original classes for clarity in shifted pipelines.

EpisodicBufferShifted = EpisodicBuffer
SASRBufferShifted = SASRBuffer
